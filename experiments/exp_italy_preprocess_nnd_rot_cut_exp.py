...
from polyagammadensity import ExponentialDensity2D, inv_exponential
...
def make_model(counts: np.ndarray) -> tuple[ExponentialDensity2D, float]:
    ny, nx = counts.shape
    ...
    prior_mean_scalar = (np.log(max(mean_count, 1e-6))- 0.5 * PRIOR_VARIANCE)
    nmax_mix = int(counts.max()) + NMAX_MIX_EXTRA
    model = ExponentialDensity2D(prior_mean=np.full(ny * nx, prior_mean_scalar), prior_precision=prior_precision, sparse=True, n=ny, m=nx, nmax_mix=nmax_mix)
    model.set_data(counts.ravel(order="C"))
    return model, prior_mean_scalar


def sample_exponential(
    model: ExponentialDensity2D,
...
def image(model: ExponentialDensity2D, values: np.ndarray) -> np.ndarray:
...
def plot_exponential_rate(
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), constrained_layout=True)
    panels = (
        (axes[0], posterior_mean_rate, "Exponential posterior mean rate"),
        (axes[1], posterior_sd_rate, "Exponential posterior rate SD"),
    )
...
def plot_exponential_f(
    ...
    panels = (
        (
            axes[0],
            posterior_mean_f,
            "Exponential posterior mean f",
        ),
        (
            axes[1],
            posterior_sd_f,
            "Exponential posterior f SD",
        ),
    )
...
def main() -> None:
    ...
    print("Italy exponential experiment")
