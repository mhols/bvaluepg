...
die glattung wird zu stark sein
"""
...
TRUTH_FILE = REPO_ROOT / "data" / "synthetic" / "2026-07-28_bars_synthetic_catalog_truth.npz"
...
def sample_laplace(model: pgd.PolyaGammaDensity2D, f_map: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    factor = pgd.cholesky(precision, lower=True)
...
def plot_comparison(
    ...
    panels = (
        ...
        (axes[1, 0], image(model, pg["mean"]), "PG mean rate", None, None),
...
'''
jetzt funktioniert der obere urspruengliche code
ich lass die Code snipets zur kontrolle mal drin, damit ich die Unterschiede sehe

# nochmal 
# warum funktioniert laplace aber pg nicht?

# lade katalog
def load_catalog() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    datafile = REPO_ROOT / "data" / "synthetic" / "2026-07-28_bars_synthetic_catalog_truth.npz"
...
# plot katalog
def plot_catalog(lambda_true: np.ndarray, counts: np.ndarray) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(10, 5), constrained_layout=True)
    panels = (
        (axes[0], lambda_true, "lambda_true"),
        (axes[1], counts, "observed counts"),
    )
    for ax, values, title in panels:
        im = ax.imshow(values, origin="lower")
        ax.set_title(title)
...
# Prior
def plot_prior(lambda_true: np.ndarray) -> None:
    fig, ax = plt.subplots(1, 1, figsize=(5, 5), constrained_layout=True)
    im = ax.imshow(lambda_true, origin="lower")
    ax.set_title("Prior (lambda_true)")
    ax.set_xticks([])
    ax.set_yticks([])
    fig.colorbar(im, ax=ax, fraction=0.046)
    plt.show()
...
def make_model(counts: np.ndarray, lambda_true: np.ndarray) -> pgd.PolyaGammaDensity2D:
    ny, nx = counts.shape
    mean_intensity = float(lambda_true.mean())
    prior_probability = float(np.clip(mean_intensity / 12.0, 1e-6, 1.0 - 1e-6))
    prior_mean_scalar = float(pgd.inv_sigmoid(prior_probability))
    prior_precision = ck.precision_matern(
        n=ny,
        m=nx,
...
# plot pg samples rate
def plot_pg_samples_rate(model: pgd.PolyaGammaDensity2D, samples_lambda: np.ndarray) -> None:
    fig, axes = plt.subplots(2, 3, figsize=(12, 7), constrained_layout=True)
    panels = (
        (axes[0, 0], model.scanorder_to_image(samples_lambda[0]), "PG sample 1"),
        (axes[0, 1], model.scanorder_to_image(samples_lambda[1]), "PG sample 2"),
        (axes[0, 2], model.scanorder_to_image(samples_lambda[2]), "PG sample 3"),
        (axes[1, 0], model.scanorder_to_image(samples_lambda[3]), "PG sample 4"),
        (axes[1, 1], model.scanorder_to_image(samples_lambda[4]), "PG sample 5"),
        (axes[1, 2], model.scanorder_to_image(samples_lambda[5]), "PG sample 6"),
    )
...
# plot pg samples f
# def plot_pg_samples_f(model: pgd.PolyaGammaDensity2D, samples_f: np.ndarray) -> None:

# plot pg mean rate
def plot_pg_mean_rate(model: pgd.PolyaGammaDensity2D, samples_lambda: np.ndarray) -> None:
    mean_lambda = model.scanorder_to_image(samples_lambda.mean(axis=0))
    fig, ax = plt.subplots(1, 1, figsize=(5, 5), constrained_layout=True)
    im = ax.imshow(mean_lambda, origin="lower")
    ax.set_title("PG mean rate")
    ax.set_xticks([])
    ax.set_yticks([])
    fig.colorbar(im, ax=ax, fraction=0.046)
    plt.show()
...
# Ablauf:
lambda_true, f_true, counts = load_catalog()
plot_catalog(lambda_true, counts)
plot_prior(lambda_true)
main_model = make_model(counts, lambda_true)
f0 = main_model.first_guess_estimator()
f_map = main_model.max_logposterior_estimator(
    f0=f0,
    niter=200,
)
samples_f, samples_lambda = sample_pg(main_model, f_map)
plot_pg_samples_rate(main_model, samples_lambda)
plot_pg_mean_rate(main_model, samples_lambda)
