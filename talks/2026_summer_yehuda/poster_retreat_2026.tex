...
\institute{
    \textit{SFB1294 Autumn School 20026 Boltenhagen}
}
...
where the precision matrix $Q$ introduces spatial dependence. And a Gamma prior is placed on the scale parameter $\lambda$.
...
\begin{theorem}
    ...
    Let $K(f'|f)$ denote the transition kernel. Let $\alpha>0$, $\delta>0$ and $Q\succ 0$. Then there exist a constant $\varepsilon>0$ and a probability density $h$ on $\mathbb{R}^M$ such that 
    \[
        K(f'|f) \geq \varepsilon h(f'), \quad f, f' \in \mathbb{R}^M
    \]
    Consequently, the marginal latent-field is uniformly ergodic
\end{theorem}
