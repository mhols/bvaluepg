...
def load_horus(df):
    ...
    required = ["datetime", "lat", "lon", "Z", "mag"]
...
def save_eqcat_mat(df, output_file):
    ...
    mat = {
        ...
        "Z": df["Z"].fillna(0.0).to_numpy(float),
        ...
        "depth": df["Z"].fillna(0.0).to_numpy(float),
    ...
    scipy.io.savemat(output_file, mat, do_compression=True)
...
def get_background_events(file_in, Mc=None, f_eta_0=None):
...
def decluster(df, Mc, f_eta_0):

    df = load_horus(df)
    df = add_time_fields(df)
    save_eqcat_mat(df, 'sprout.mat')
    return get_background_events('sprout.mat', Mc, f_eta_0)
