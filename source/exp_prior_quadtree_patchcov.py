...
2. Constructs a patch-based Gaussian prior on the quadtree rectangles.
...
def rectangle_subgrid_points(
    ...
    """Return an m x m midpoint grid inside a rectangle."""
...
def rbf_kernel_points(
    P: np.ndarray,
    Q: np.ndarray,
    rho: float,
    v2: float,
) -> np.ndarray:
    """Pairwise RBF kernel between two point clouds."""
    diff = P[:, None, :] - Q[None, :, :]
    dist2 = np.sum(diff**2, axis=2)
    return v2 * np.exp(-dist2 / (2.0 * rho**2))


def patch_covariance_rectangles(
    row_i: pd.Series,
    row_j: pd.Series,
    rho: float,
    v2: float,
    ...
    """Approximate patch covariance between two quadtree rectangles.

    This approximates
        (1 / (|Omega_i| |Omega_j|)) integral_{Omega_i} integral_{Omega_j} K(x, x') dx dx'
    by averaging the kernel over midpoint subgrids inside both rectangles.
    """
    Pi = rectangle_subgrid_points(
        float(row_i["xmin"]),
        float(row_i["xmax"]),
        float(row_i["ymin"]),
        float(row_i["ymax"]),
    ...
    Kpq = rbf_kernel_points(Pi, Pj, rho=rho, v2=v2)
    return float(Kpq.mean())


def patch_covariance_matrix(
    ...
    """Build full patch covariance matrix for the quadtree rectangles."""
    n = len(grid_df)
    Sigma = np.zeros((n, n), dtype=float)

    for i in range(n):
        row_i = grid_df.iloc[i]
        for j in range(i, n):
            row_j = grid_df.iloc[j]
            cij = patch_covariance_rectangles(row_i, row_j, rho=rho, v2=v2, m=m)
            Sigma[i, j] = cij
            Sigma[j, i] = cij

    Sigma += jitter * np.eye(n)
    return Sigma
...
def make_prior_model(grid_df: pd.DataFrame, nobs: np.ndarray) -> tuple[PolyaGammaDensity, np.ndarray, np.ndarray, int]:
    ...
    if USE_PATCH_COVARIANCE:
        Sigma0 = patch_covariance_matrix(
            grid_df,
            rho=RHO,
            v2=v2,
            m=PATCH_SUBGRID_M,
        ...
        raise NotImplementedError(
            "This patchcov experiment script is meant to use the patch-based covariance."
        )
...
def main():
    ...
    print(f"lam = {lam}")
    print(f"use patch covariance = {USE_PATCH_COVARIANCE}")
    print(f"patch subgrid m = {PATCH_SUBGRID_M}")
    ...
    plot_covariance_and_correlation(Sigma0, selected_cells)
